...
load("ringed_nest_for_simulation.Rda") 

n.marked = ringed_nest_for_simulation[-c(1),] # Number of marked individuals each year and site
...
phi1 = 0.20 # First year
phiA = 0.85 # Subadult and adult
phi = c(phi1, phiA)

# Productivity
rhoLR = 1.7/2 # La Ronze
rhoSAT = 0.6/2 # Satellite colonies
rho = c(rhoLR, rhoSAT)

# Recruitment probability
kappaLR = 1 # La Ronze
kappaSAT = 1 # Satellite colonies
...
p_CV <- 0.5
a_p <- 1/(p_CV*p_CV) 
b_p <- (1/(p_CV*p_CV))*(1/p_mean)

p = matrix(rgamma(n = n.years*n.colony, a_p,b_p),
           nrow = n.years, ncol = n.colony)

# Simulate some years with no reading effort
p[rbinom(length(p), size = 1, prob = 0.1) == 1] <- 0
...
for (t in 1:(n.years-1)) {
  ...
  extinted_col = sample(x = c(2:4), size = rpois(1, 0.4)) %>% as.vector()
  #extinted_col
