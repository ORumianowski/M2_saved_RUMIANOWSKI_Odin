...
cat(file = "2_models/2_models_for_real/v16_fecundity/models/v16_2.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  for (col in 1) {
    for (t in 1:(n.years-1)){
        rho[col, t] <- rho_LR1 * equals(active_fecundity[col, t], 1) + rho_LR2 * equals(active_fecundity[col, t], 2)
    }
  }


  for (col in 2:n.colony) {
    for (t in 1:(n.years-1)){
        rho[col, t] <- rho_SAT1 * equals(active_fecundity[col, t], 1) + rho_SAT2 * equals(active_fecundity[col, t], 2)
    }
  }
