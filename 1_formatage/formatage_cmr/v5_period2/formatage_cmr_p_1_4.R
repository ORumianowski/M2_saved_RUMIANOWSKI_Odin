...
site_period1 = c("LR_E", "MA_E",
                 "PC_E", "V5_E")

site_period2 = c("LR_E", "MA_E",  
                 "VS_E", "LA_E","SV_E", "WV_E")

site_period3 = c("LR_E", "MA_E",  
                 "VS_E", "LA_E","SV_E", "WV_E", 
                 "SZ_E")

# Our period --------------------------------------------------------------
our_sites = site_period1
our_period = 1986:2006
...
dgull = dgull0 %>%  
  subset(., annee %in% our_period)%>% 
  mutate(site_ch = if_else(id_site_principal %in% our_sites, id_site_principal, "AE"))
...
dgull_final = dgull_final %>%  
  subset(., site_ch %in% our_sites | age == "P")
...
dgull_final <- dgull_final %>% 
  mutate(site= recode(site_ch, !!!setNames(as.character(as.integer(1:n.colony)), c(our_sites, "AE")))) %>%  
