...
# La partie 1 contient les manipulations spécifiques à chaque jeux de données CH et à la contruction de leur m-array associé. Deux manipulations nécessaires pour rendre comptatible le jeux de données au modèles ont été nécéssaire : les deux poussins fictifs et la supressions des lectures sur des colonies absentes.
# 
# La partie 2 contient les manipulations nécaissraires pour construire traiter les survey pour chaque période : sélections des années et des colonies suivies 
...
# Deux poussins fictifs (sans effet sur les estimations car contacté la dernière années) néssaire à la contruction du marray
CH = rbind(CH,
           matrix(c(0,0,0, 3), ncol = 4),
           matrix(c(0,0,0, 4), ncol = 4)) save(CH, file = "CH_p3_4_assec_2805.Rda")
n.colony = 7
y = CH 
marr = IPMbook::marray(y, unobs=n.colony+1) 
...
# Suppression des lecture de bagues sur colonies absentes -----------
